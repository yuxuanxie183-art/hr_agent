...
atexit.register(close_db, db_conn)  #注册进程退出时的清理钩子hook，只要不断电、kill-9都可以自动关闭数据库资源
#预留文件输出
...
def get_employee_profile(uid:str)->str:
    """
    根据员工UD查询员工的完整人事档案，包括姓名、职级、工作城市、入职年限、基本性质。当需要获取当前对话员工的背景属性时，必须首先调用此工具
    """
    ...
    if not res:
        return f'错误！ ：未找到 UID为{uid}的员工信息'
    employee = res[0]
...
def get_leave_balance(uid: str) -> str:
    """
    根据员工 UID 查询剩余的假期余额（包括年假和病假）。
    当员工明确询问「我还有几天假」或「我的假期余额」时调用。
    """
    sql = """
    SELECT a.name,
    b.annual_leave_remaining,
...
def generate_employment_certificate(uid:str, cer_type:str) -> str:
    ...
    if cer_type == 'income':
        ...
        if rank_level < 5:
            return (f'系统提示：根据公司规定，P4以下职级员工（当前员工为{employee['rank']}）无法线上开具薪资收入证明。'
                f'请引导员工在线人工提交工单，由HR线下手动核查开具')
        content = (
        ...
        return (f'[系统提示]已经自动为您生成收入证明\n-----------------------\n'
                f'{content}'
                f'\n-----------------------'
                )
    elif cer_type == 'employment':
        content = (
            f'《薪资收入证明》\n兹证明我公司员工{employee['name']},职级为{employee['rank']}。特此证明。\n'
            f'\n （公章）'

        )
        return (f'[系统提示]已经自动为您生成在职证明\n-----------------------\n'
                f'{content}'
                f'\n-----------------------'
                )
